...
## Current State: v32.9 EditQuestion Option-Edit Data Integrity (Identity-Based) — ✅ SHIPPED local + audited PASSED + CLOSED + tagged `v32.9` 2026-06-25 (1 fase/420, 3 plan, 6/6 REQ OPTEDIT-01..05 + VRF-01, **migration=FALSE**, branch main, **NOT PUSHED**). Fix backlog 999.15: upsert opsi EditQuestion posisional → identity-based (match `PackageOption` by stable Id) = no silent relabel jawaban peserta saat hapus opsi tengah pada soal terjawab; suite 702/702 + Playwright UAT 3/3 real-browser + integrasi SOUND. Prev: v32.6 Section + Scoped Shuffle + Pagination + Opsi Dinamis ✅ CLOSED 2026-06-24 (20/20 REQ, **migration=TRUE** Phase 415 `AddAssessmentPackageSection`); v32.5 Flexible Add/Remove Participant ✅ CLOSED 2026-06-22 (migration=TRUE Phase 409); v32.2 Inject ✅ CLOSED 2026-06-19; v32.0 ✅ CLOSED manual; v31.0 merged origin/main. **Next: `/gsd-new-milestone` + push bundle origin/main + notify IT (migration=TRUE: 415 [v32.6] + 409 [v32.5] di range unpushed sama; v32.9 migration=FALSE; + carry 360/372).**

## Current Milestone: v32.9 EditQuestion Option-Edit Data Integrity (Identity-Based)

**Goal:** Hapus/edit opsi jawaban pada soal yang SUDAH dijawab peserta tidak lagi me-relabel jawaban peserta secara senyap. Ganti upsert opsi POSISIONAL di `AssessmentAdminController.cs` EditQuestion POST menjadi **IDENTITY-based** — cocokkan baris input ke `PackageOption` existing by stable `Id`, bukan posisi — sehingga menghapus opsi tengah membuang record yang BENAR dan guard answered-option (D-418-02) menyala untuk delete posisi MANAPUN.

**Target features (fase mulai 420):**
- **Identity-based option upsert** — form authoring `ManagePackageQuestions` kirim hidden `OptionId` per baris; controller match input→existing `PackageOption` by Id (bukan `existing[i]` posisi); hapus opsi tengah me-Remove record yang tepat.
- **Guard answered-option universal** — guard D-418-02 menyala untuk penghapusan opsi yang sudah dijawab di posisi MANAPUN (bukan cuma ekor); pesan ramah, no silent relabel.
- **Regression-lock 999.14** — konversi MC/MA→Essay + penyusutan-ekor pada soal terjawab tetap diblokir (no FK-Restrict 500).
- **Test + UAT** — integration test controller-level yang mereproduksi relabel-senyap lalu membuktikan kini diblokir + Playwright UAT real-browser (lesson 354).

**Konteks kunci:** Sumber = backlog 999.15 (review Phase 418 WR-01), diverifikasi reproduces on main 2026-06-24 (7-agen verify workflow). Bug: `PackageUserResponse` simpan `PackageOptionId` saja (no text snapshot); upsert posisional (`AAC:8121-8160`) pertahankan `Id` by-posisi → makna jawaban peserta berubah senyap di review/grading/PDF. **migration=FALSE**; branch main; fase mulai **420** (lanjut dari 419). ⚠️ In-code note `AAC:8027-8035` peringatkan upsert posisional di-LOCK spec D-418-02 → fix ubah **MEKANISME** (identity-based), BUKAN sekadar perketat threshold guard. v32.8 ada di branch ITHandoff. Backlog housekeeping: drop 999.14 (closed 418) / 999.9 (no-op, view dihapus 396) / 999.5 (stale, 365 covered); 999.13 + 999.16 permanent-defer (defensif, no-op/benign). OUT: data-repair response historis (defer s/d ada bukti korupsi nyata).

**v32.2 shipped (local) + audit PASSED + closed (2026-06-19)** — Inject Hasil Assessment Manual "Seakan Online" (phases 393-398 + 398.1, 7 phase / 26 plan, 13/13 REQ INJ-01..13, **0 migration**): page baru `/Admin/InjectAssessment` (Section C, Admin+HC) meng-inject hasil assessment manual identik online — wizard 6-langkah (setup room + authoring soal reuse ManagePackages + worker picker + sertifikat 3-mode), `InjectBatchAsync` byte-identik online (reuse GradingService+AssessmentScoreAggregator+CertNumberHelper, NOL duplikasi pipeline), mode jawaban input-asli/auto-generate, import Excel matrix (+retire BulkBackfill), link Pre/Post silang inject↔online (cross-grouping §13 utuh, online tak disentuh). E2E inject tampil identik online (/CMP/Records + Results per-soal + sertifikat). **398.1 tech-debt cleanup**: 8 FIX + 2 DROP-with-evidence (dup-NIP GroupBy, g.First→OrderBy deterministik, PendingGrading edit-guard, LBL-02, download debounce, de-tautologize 391 test, fix flaky essay e2e helper). Re-audit-2 PASSED (7/7 phase VERIFICATION, integration 7/7 WIRED 0-regression, nyquist 6/6, tech-debt RESOLVED). suite 391/0 + e2e essay 17/17. Archive: `milestones/v32.2-*` + `milestones/v32.2-phases/`. **branch main, NOT PUSHED** (deploy bareng — notify IT migration=FALSE).
...
**Current focus:** v32.5 Flexible Add/Remove Participant — 🚧 mendefinisikan requirements + roadmap (fase mulai 409), design spec `docs/superpowers/specs/2026-06-19-flexible-add-remove-participant-design.md` (committed ccdc78ef, migration=TRUE). v32.2 Inject Assessment ✅ SHIPPED+CLOSED 2026-06-19. Sebelumnya v32.0 (391+392) **KEDUA PHASE ✅ COMPLETE 2026-06-17** (391 + 392, 7/7 REQ, **0 migration**, branch main). **Phase 391** Penambahan Peserta Fleksibel (PART-01..04): DeriveReadyStatus siap-mulai + guard by-window + lindungi sesi berjalan + notice Info; verifier 8/8 + 486/486 test + code-review WR-02 fix + UAT browser 2/2 + secure 8/8 + nyquist-compliant; bonus fix blocker AssessmentType NOT NULL. **Phase 392** Perbaikan CreateWorker + Audit Field (WRKR-01..03, VIEW-ONLY): unlock Nama/Email + reword AD + type=email + validasi inline+live; verifier 10/10 + e2e 3/3 + code-review WR-01 fix; sisa: secure-phase 392 + validate-phase 392 + UAT AD-on Dev. Sisa milestone: `/gsd-audit-milestone` + notify IT (migration=FALSE). v31.0 CLOSED + merged origin/main 2026-06-16. Carry-migration IT lama (360 PendingProtonBypass + 372 ShuffleToggles) masih pending notify.

## Latest Shipped Milestone: v32.6 Section + Scoped Shuffle + Section Pagination + Opsi Dinamis — ✅ SHIPPED local + CLOSED 2026-06-24 (6 fase 415-419 + 415.1, 20/20 REQ, milestone audit PASSED, migration=TRUE Phase 415, NOT PUSHED)

**Goal:** HC/Admin dapat mengelompokkan soal ke dalam **Section** (per area/equipment) di tiap paket, mengacak soal & pilihan **hanya di dalam lingkup section** (scoped shuffle, on/off per-section), mengatur **pagination per-section** (section tertentu mulai halaman baru), memakai **jumlah opsi jawaban dinamis (2–6)** (bukan kunci A–D), dan mengunggah semua via **import Excel** yang diperluas. Section **opsional** → kosong = perilaku global lama (100% kompatibel-mundur). Design spec: `docs/superpowers/specs/2026-06-22-section-scoped-shuffle-pagination-dynamic-options-design.md` (+ §15 addendum re-check; commit `93fdb55a`+`26c16337`). **migration=TRUE** (tabel `AssessmentPackageSection` + kolom `PackageQuestion.SectionId` nullable).

**Target features (fase mulai 415):**
- **Section foundation** — entity `AssessmentPackageSection` per-paket (No.Section, Nama, StartNewPage, ShuffleEnabled) + `SectionId` nullable di soal + UI kelola/urut/toggle section + import kolom No.Section/Nama + validasi struktur-section antar-paket (tolak keras D-13).
- **Scoped shuffle** — generalisasi `ShuffleEngine` jadi acak per-section (kunci ET komposit `(Section,ET)`; tanpa section = perilaku sekarang) + precedence toggle assessment(induk)/section(anak).
- **Section pagination** — toggle "Mulai Halaman Baru" per-section + render header section + auto-pecah per-10 + resume map; default 10/halaman mengalir.
- **Opsi jawaban dinamis 2–6** — refactor authoring (CreateQuestion/EditQuestion + form + inject form) + render ujian + preview + results (huruf A–F) + import (Opsi A–F, ExtractPackageCorrectLetter A–F, min 2/max 6).
- **Import Excel diperluas** — template + parser kolom Section + Opsi A–F, dual-format kompatibel-mundur (file 9-kolom lama tetap jalan) + fingerprint dedup +Section+opsi.

**Konteks kunci:** 15 keputusan terkunci (D-01..D-15 di spec). D-03 Section di-dalam-paket; D-09 >1 paket = pool section padanan antar-paket lalu sampling dalam batas section (syarat jumlah-soal/section sama); D-12 TANPA breakdown skor per-section; D-13 mismatch struktur section = tolak keras (validasi saat import + sebelum mulai ujian); D-14 toggle assessment=induk per-section=anak; D-15 soal tanpa section → grup "Lainnya" di akhir. AMAN (2 gray-area sweep, 14 agen, 0 fatal): skor%/Lulus immutable, grading by `PackageOption.Id`, breakdown ET independen, shuffle-lock final. Interaksi lintas-milestone: Inject v32.2, LinkPrePost 397, Add/Remove peserta v32.5, Retake v32.4 (ITHandoff), MultiUnit v32.3 (ITHandoff). Branch main; fase mulai **415** (lanjut dari 414, tak reset). OUT: quick-win Excel dropdown/auto-skor (milestone terpisah), tipe soal baru (Matching/FileUpload), breakdown skor per-section.

## Previous Milestone: v32.5 Flexible Add/Remove Participant — ✅ SHIPPED local + CLOSED 2026-06-22 (audit PASSED 11/11 REQ; +Phase 414 off-theme; migration=TRUE Phase 409; NOT PUSHED)

**Goal:** Admin/HC dapat menambah & menghapus peserta pada assessment **kapan saja** — baik batch yang belum ada progres peserta maupun yang sedang berjalan (InProgress) — langsung dari layar Monitoring Detail (live, AJAX+SignalR). Hapus **hybrid by-state**: peserta belum-mulai → hard-delete bersih; peserta sudah-ada-data/Completed → soft-remove + arsip (reversibel, sertifikat aman). Design spec: `docs/superpowers/specs/2026-06-19-flexible-add-remove-participant-design.md`.

**Target features (fase mulai 409):**
- **Data + backend core** — migration 3 kolom `RemovedAt/RemovedBy/RemovalReason`; service hard-delete (not-started) / soft-remove (ada-data); guard re-entry `StartExam`/`SubmitExam`/`Hub.JoinBatch`; filter `RemovedAt!=null` di semua list/count.
- **Add peserta live** — endpoint `AddParticipantsLive` (guard window/idempotent/auto `UserPackageAssignment`, status siap-mulai) + dukung pasangan Pre/Post.
- **Remove + Restore live** — endpoint `RemoveParticipantLive` (hybrid + Pre/Post pair-as-unit) + `RestoreParticipantLive` + perbaiki stub mati `DeleteAssessmentPeserta`.
- **Monitoring UI + SignalR** — kontrol Tambah/Hapus di Monitoring Detail, modal konfirmasi keras, event `participantAdded`/`participantRemoved` + `examRemoved` force-kick, panel "Peserta Dikeluarkan".
- **Test + UAT** — xUnit integration (pola `FlexibleParticipantAddTests`) + Playwright e2e live.

**Konteks kunci:** branch main; **migration=TRUE** (additif 3 kolom nullable); RBAC Admin+HC penuh (longgarkan `EnsureCanDeleteAsync` HC, mitigasi = konfirmasi keras + audit `RemovedBy`); soft-remove sumber-kebenaran `RemovedAt != null`; scope OUT Proton/self-service/bulk-import-live; fase mulai **409** (hindari tabrak v32.3/v32.4 di ITHandoff). Catatan: DELETE 1-peserta backend belum ada (stub `DeleteAssessmentPeserta` mati di `EditAssessment.cshtml:666`).

## Earlier Milestone: v32.2 Inject Hasil Assessment Manual ("Seakan Online") — ✅ SHIPPED local + CLOSED 2026-06-19

**Goal:** HC/Admin dapat meng-inject hasil assessment manual (ujian offline/kertas, data migrasi, acara lisensor luring) yang **identik dengan hasil online** — muncul di riwayat pekerja, rincian jawaban per-soal, breakdown elemen teknis, sertifikat opsional — via page baru `/Admin/InjectAssessment` (Kelola Data Section C, Admin+HC). Mesin existing (authoring soal + `GradingService` + `CertNumberHelper`) di-**reuse** (nol duplikasi logic) → skor/lulus/cert dihitung identik online. **0 migration** (semua tabel sudah ada). Design spec: `docs/superpowers/specs/2026-06-17-inject-assessment-manual-design.md`.

**Target features (6 fase, 393-398):**
- **393 Backend core inject** — `InjectAssessmentService` membangun session set lengkap (`AssessmentSession`+`UserPackageAssignment`+`PackageUserResponses`+`SessionElemenTeknisScore`+cert) lewat reuse `GradingService`/`FinalizeEssayGrading`, atomic per-batch, AuditLog "ManualInject"; xUnit.
- **394 Page + Setup Room + authoring soal** — controller+view berlangkah, form setting mirror `CreateAssessment` (+ cert toggle: auto/manual/tanpa), reuse partial authoring soal MC/MA/Essay, worker picker, card menu Section C, RBAC Admin+HC.
- **395 Mode jawaban** — input jawaban asli per pekerja (form) + auto-generate pola jawaban dari skor target (MC/MA; essay set skor langsung).
- **396 Import Excel + retire BulkBackfill** — template generator + matrix parser (baris=NIP, kolom=soal) atomic; pensiun/redirect tool lama `BulkBackfill`.
- **397 Link Pre/Post ke room existing** — search picker room (reuse `ManageAssessmentTab_Assessment`) + wiring `LinkedGroupId`/`LinkedSessionId`, dukung silang inject↔online (Pre inject ↔ Post real).
- **398 Test + UAT "seakan online"** — E2E full lifecycle (inject → /CMP/Records + /CMP/Results per-soal + sertifikat), regression, audit milestone.

**Konteks kunci:** Anchor paket = pola sentinel (`CMPController.cs:1034-1090`): paket di-anchor ke sesi representatif room, tiap pekerja `UserPackageAssignment(AssessmentPackageId=sentinel + ShuffledQuestionIds)`; grading & Results baca soal **by question ID** via `ShuffledQuestionIds`. `GetUnifiedRecords` (`WorkerDataService.cs:28`) **tak** filter `IsManualEntry` → visibility /CMP/Records gratis. Transparansi: `IsManualEntry=true` + AuditLog wajib. Branch main; 0 migration; lanjut nomor fase dari 392 (tak reset).

## Earlier Milestone: v32.0 Manajemen Peserta — ✅ Phases 391+392 COMPLETE (local, belum diarsip) 2026-06-17

**Goal:** HC mengelola peserta assessment lancar — penambahan peserta fleksibel saat ujian berjalan (391, PART-01..04) + `/Admin/CreateWorker` bisa dipakai lagi + audit field view-only (392, WRKR-01..03). 7/7 REQ, **0 migration**, branch main. Belum `/gsd-complete-milestone` (dir phases 391/392 masih live). Sisa: secure/validate 392 + audit-milestone + notify IT (migration=FALSE).

## Earlier Milestone: v31.0 Hotfix Pra-Ujian Lisensor — ✅ SHIPPED + CLOSED + merged origin/main 2026-06-16
...
## Older Milestone: v30.0 Essay Grading Correctness + Monitoring UI Refactor — ✅ SHIPPED + CLOSED 2026-06-15
...
*Last updated: 2026-06-24 — milestone **v32.9 EditQuestion Option-Edit Data Integrity (Identity-Based)** STARTED (mendefinisikan requirements + roadmap, fase mulai 420, migration=FALSE, branch main). Scope = backlog 999.15 (relabel jawaban senyap saat hapus opsi tengah pada soal terjawab) diverifikasi reproduces 2026-06-24; backlog lain di-drop/defer. Sebelumnya v32.6 Section + Scoped Shuffle + Pagination + Opsi Dinamis SHIPPED local + CLOSED 2026-06-24 (6 fase 415-419 + 415.1, 20/20 REQ, migration=TRUE Phase 415, NOT PUSHED). v32.5 SHIPPED+CLOSED 2026-06-22 (migration=TRUE Phase 409). v32.2 Inject shipped+closed 2026-06-19. v31.0 merged origin/main 2026-06-16. Sisa koordinasi: push origin/main (bundle v32.2+v32.5+v32.6) + notify IT (migration=TRUE 415+409).*
