...
- [ ] **MAM-07**: Tab2 pagination (Skip/Take + kontrol) atau drop param mati `page/pageSize` (`AssessmentAdminController.cs:245`). (filter)
- [ ] **MAM-08**: Delete Training/ManualAsm preserve filter context — `hx-post` re-swap (`_TrainingRecordsTab.cshtml:327-349`). (function)
- [ ] **MAM-09**: Filter Status Tab2 relabel "Status Training" / lipat passed assessment (`_TrainingRecordsTab.cshtml:107-125` + `WorkerDataService.cs:390`). (ui-design)
