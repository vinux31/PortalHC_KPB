...
| 408 Test & UAT | RTK-14 | 🚧 ARTEFAK 2/2 — SISA LIVE UAT + SECURE GATE (408-01 ✅ xUnit integration GAP-1 `RetakeThenPassCertTests` — retake→grade-lulus→**TEPAT 1 cert** [anti-double-cert guard + unique index] + format `KPB/\d{3}/[IVX]+/\d{4}`, GREEN @SQLEXPRESS, REUSE RetakeServiceFixture, 0 test hijau tersentuh, commit `39e3ef46`; counting (UserId,Title,Category) no-conflate Pre/Post = `RetakeServiceTests.Counting_PrePostSameTitle_NoConflate` existing. 408-02 ✅ ARTEFAK: Playwright lifecycle `retake-lifecycle-408.spec.ts` (1 happy-path real-browser gagal→Ujian Ulang→jawab benar→LULUS + Nomor Sertifikat, leak-safe pra-retake + 0 pageerror) + seed `retake-lifecycle-408-seed.sql` ([RETAKE408] cooldown=0 + GenerateCertificate=1 + paket soal jawaban-benar deterministik), `dotnet build` 0 err + spec `--list` parse OK, 0 file existing tersentuh, commit `5159e7d0`+`0497c18b`. **SISA RTK-14**: (1) live UAT lifecycle penuh @5270 + full regresi e2e/xUnit [Task 3 orchestrator autopilot gate], (2) security gate `gsd-secure-phase 408` [RBAC/antiforgery/cooldown-cap revalidation/no-leak]. migration=FALSE) |
