# Requirements: Portal HC KPB

**Defined:** 2026-02-19
**Core Value:** Evidence-based competency tracking with automated assessment-to-CPDP integration

## v1.4 Requirements

Requirements for Assessment Monitoring milestone. Phases continue from 15 → start at 16.

### Assessment Monitoring (MON)

- [ ] **MON-01**: Monitoring tab groups sessions by assessment (Title + Category + Schedule.Date) — one row per group instead of one row per user
- [ ] **MON-02**: Each group row shows completion progress — "X/Y completed" with a visual progress bar
- [ ] **MON-03**: Each group row shows pass rate — "Z passed (N%)" based on completed sessions
- [ ] **MON-04**: HC can expand a group row to see per-user detail: user name, status (Not started / In progress / Completed), score, pass/fail
- [ ] **MON-05**: Monitoring includes recently closed assessments (closed within last 30 days) in addition to Open + Upcoming
- [ ] **MON-06**: Groups are sorted by schedule date (soonest first for Open/Upcoming, most recent first for closed)

## Future Requirements

*(Nothing deferred yet)*

## Out of Scope

| Feature | Reason |
|---------|--------|
| Search/filter on monitoring tab | Not requested for this milestone |
| Export from monitoring | Reports dashboard handles exports |
| Individual session actions from monitoring | Management tab handles edits |

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| MON-01 | Phase 16 | Pending |
| MON-02 | Phase 16 | Pending |
| MON-03 | Phase 16 | Pending |
| MON-04 | Phase 16 | Pending |
| MON-05 | Phase 16 | Pending |
| MON-06 | Phase 16 | Pending |

**Coverage:**
- v1.4 requirements: 6 total
- Mapped to phases: 6
- Unmapped: 0 ✓

---
*Requirements defined: 2026-02-19*
