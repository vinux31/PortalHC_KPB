...
- ✅ **v32.8 Exam Security & Audit Hardening** — Phases 426-428 (shipped local + audited PASSED + closed 2026-06-25, 3/3 REQ AUDIT-01 + EXSEC-01/02, integration 6/6 SOUND; **migration=TRUE hanya Phase 427** [`AddTokenVerifiedAt`]; branch ITHandoff, NOT pushed [bundle dgn v32.1+v32.3+v32.4+v32.7]) — [archive](milestones/v32.8-ROADMAP.md) — [requirements](milestones/v32.8-REQUIREMENTS.md) — [audit](milestones/v32.8-MILESTONE-AUDIT.md)

## Phases

<details>
<summary>✅ v32.8 Exam Security & Audit Hardening (Phases 426-428) — SHIPPED local + audited PASSED + closed 2026-06-25 (3/3 REQ AUDIT-01 + EXSEC-01/02, integration 6/6 SOUND, migration=TRUE Phase 427 only) — <a href="milestones/v32.8-ROADMAP.md">archive</a> — <a href="milestones/v32.8-REQUIREMENTS.md">requirements</a> — <a href="milestones/v32.8-MILESTONE-AUDIT.md">audit</a></summary>
...
- [x] **Phase 425: Cosmetic / Naming / Tech-Debt Cleanup (CLN-01..05)** ✅ COMPLETE 2026-06-24 (verify 5/5 + code-review 0C/0W/1I + suite 768/0/2 + build 0err) — batch low-risk naming/redundancy/dead-field/dokumentasi. CLN-01 label & doc diselaraskan (RESERVED XML-doc AssessmentPhase, Status 7-nilai, [Display] ValidUntil, koreksi FK app-level, sentinel AssessmentPackageId); CLN-02 entry manual cross-validate IsPassed vs Score/PassPercentage = PERINGATAN non-blocking (`ManualEntryRules.PassStatusMismatch` + `TempData["Warning"]`, TETAP simpan); CLN-03 dead-field `AssessmentPhase` ditandai **RESERVED** (BUKAN drop, D-01); CLN-04 timer satu sumber → `ExamTimeRules.AllowedExamSeconds` (4 situs + parity test); CLN-05 konvensi ModelState via `ControllerGuards.JsonFail` (subset SubmitEssayScore, D-04). **migration=FALSE** (D-01 RESERVED + D-03 timer aman). **DEFER ke backlog (D-03):** FLOW-08 token server-authoritative (`TokenVerifiedAt`) + FLOW-10 write-on-GET StartExam refactor + DROP kolom AssessmentPhase + refactor DTO penuh ModelState.
...
- [x] 421-03-PLAN.md — Wave 3: Guard hapus peserta ber-riwayat server round-trip + cleanup arsip cascade (RTH-04) + modal pra-simpan MaxAttempts<terpakai & warning cooldown>window (RTH-05/D-02) + UAT @5270 [migration=FALSE, autonomous=false]; depends 02
**UI hint:** yes
...
- [x] 423-01-PLAN.md — Wave 1 [KEYSTONE]: NEW Helpers/CertIssuanceRules.cs (ShouldIssueCertificate · DeriveValidUntil · ResemblesAutoCertFormat · PendingAgeBadgeClass) + harden CertNumberHelper.TryAssignNextSeqAsync (retry+jitter cap 8) + Wave-0 pure tests (CERT-01/02/03/06) [migration=FALSE]
- [x] 423-02-PLAN.md — Wave 2: wire helper ke 4 cert-issue site (GradingService :287/:520, FinalizeEssayGrading :3887, AddManualAssessment :759) + stop hardcode + namespace reject + try/catch friendly + anti-dup guard unconditional HasActiveCertForTitleAsync + integration tests real-SQL (CERT-01/03/04/05); depends 01 [migration=FALSE]
- [x] 423-03-PLAN.md — Wave 3 [autonomous=false]: badge umur PendingGrading di EssayGrading.cshtml + AssessmentMonitoringDetail.cshtml (PendingAgeBadgeClass, >3 kuning >7 merah, NO auto-finalize) + Playwright smoke + checkpoint UAT @5270 (CERT-07); depends 01+02 [migration=FALSE]
**UI hint:** yes
...
- [x] 424-02-PLAN.md — Wave 2: CMPController — gate gating Pre→Post StartExam (GRDF-01) + pairing terfilter UserId (GRDF-03) + clamp ExtraTime :469 (GRDF-05) + essay kosong on-time reject (GRDF-07) + real-SQL gate/essay tests; depends 01 [migration=FALSE]
- [x] 424-03-PLAN.md — Wave 3 [autonomous=false]: AssessmentAdminController — matikan auto-link judul Standard forward-only (GRDF-04/D-08) + konfirmasi export Durasi Aktual (GRDF-05) + checkpoint UAT live @5270 (gating/essay/export/forward-only/smoke); depends 01+02 [migration=FALSE]
**UI hint:** yes
...
**Status:** 🚧 ACTIVE 2026-06-21 — requirements promoted (`v32.4-PLANNED-REQUIREMENTS.md` → `REQUIREMENTS.md`), roadmap activated, PROJECT/STATE reset. Spec + plan 405 committed 2026-06-19. Phase 405 plan READY; 406/407/408 belum ditulis.
...
**Plan:** Phase 405 ditulis `docs/superpowers/plans/2026-06-19-v32.4-phase-405-backend-core.md` (9-task TDD, kode riil). Phase 406/407/408 belum ditulis (`/gsd-plan-phase` saat siap).
**Requirements:** `.planning/REQUIREMENTS.md` (RTK-01..14, traceability fase).
...
- [x] 405-04-PLAN.md — Wave 4: refactor ResetAssessment→delegasi service + clear token + UpdateRetakeSettings endpoint (RBAC+AntiForgery+sibling propagation+clamp+audit+PRG) + bulk-add carry savedAssessment + ManagePackages ViewBag (RTK-06/04/01)
**UI hint:** no
...
- [x] 407-01-PLAN.md — Wave 1: tier helper pure RetakeRules.ResolveReviewMode (enum 3-state leak-safe) + perluas AssessmentResultsViewModel/AllWorkersHistoryRow + 5 unit test cabang truth-table (RTK-11/12) [migration=FALSE]
- [x] 407-02-PLAN.md — Wave 2: CMPController inject RetakeService + action RetakeExam (antiforgery+ownership Forbid+CanRetakeAsync re-check+clear-token+redirect StartExam) + Results VM populate (flags+riwayat load) + RetakeExam endpoint test (RTK-09/10/12/13); depends 407-01
- [x] 407-03-PLAN.md — Wave 3: Results.cshtml tier 3-state leak-safe + retake control (tombol/counter/cooldown countdown/lock) + modal antiforgery + riwayat card + _RiwayatPekerja partial ter-gate + Records trigger + Playwright smoke leak-safety @5270 [checkpoint UAT]; depends 407-01+407-02 (RTK-10/11/12)
**UI hint:** yes
...
- [x] 399-04-PLAN.md — Wave 2: display semua unit 7 surface (badge primary + _PSign all-units + Excel) + AccountController/HomeController populate + Playwright (MU-03)
**UI hint:** yes
...
- [x] 401-05-PLAN.md — Wave 1: CDP filter-axis swap ×4 + defensive resolver drop-fallback ×2 + read-path skip/warn (PSU-01/02/05)
- [x] 401-06-PLAN.md — Wave 1: ProtonData BypassList filter-axis + BypassSave TargetUnit ∈ worker.UserUnits + org-tree (PSU-02/03)
- [x] 401-03-PLAN.md — Wave 2: CoachMapping ∈UserUnits validation (Assign/Edit/Import) + Cleanup/Import-reactivate no-clobber + Reactivate guard + D-01 UI indicator (PSU-03/04/05/07) [checkpoint]
...
- [x] 402-02-PLAN.md — Wave 1: backend assign — CXU-02 cross-Bagian server guard + CXU-03 per-coachee unit loop (∈ org-tree ∩ coachee.UserUnits) + CXU-01 ViewBag.CoacheeUnits dict (CXU-01/02/03)
- [x] 402-03-PLAN.md — Wave 1 (PARALEL): CDP self-scope union — stop forcing primary :305/:326/:647 + coach.UserUnits AvailableUnits + UnitFilterEnabled flag + dropdown enable + UAT checkpoint (CXU-05)
- [x] 402-04-PLAN.md — Wave 2 (depends 02): assign modal UI — IC-1 coach-first auto-scope + IC-2 per-row unit select + IC-3 relax lock + IC-4 payload map + e2e fill + UAT checkpoint (CXU-01/03/04)
**UI hint:** yes
...
- [x] 404-04-PLAN.md — Wave 3: UAT browser PROTON sekuensial cross-unit @5270 (snapshot→seed→restore) + IT handoff HTML (migration=TRUE Phase 399) + D1=b markdown (QA-02) [checkpoint, migration=FALSE]
...
| 404. Test (SQL Riil) + UAT + Docs + Invariants (QA-01..04) | 4/4 | Complete   | 2026-06-21 |
...
- [x] 383-02-PLAN.md — Wire CMPController.Results 3 titik + IsEssayPending (D-06) + D-07 TextAnswer + blok view Results.cshtml + regression count==N+2 (ECG-02/03/04; closes RES-02)
- [x] 383-03-PLAN.md — PDF export unify ke `IsQuestionCorrect` (essay >0, D-03) di AssessmentAdminController (ECG-05)
- [x] 383-04-PLAN.md — Regression real-SQL lock SubmitEssayScore persist+range+authz & FinalizeEssayGrading recompute-incl-essay+idempotent, no code change (ECG-06)
**UI hint:** yes
...
- [x] 384-04-PLAN.md — Wave 3: aktifkan e2e FLOW 384 hijau (round-trip) + UAT manual checkpoint (UIG-04)
**UI hint:** yes
