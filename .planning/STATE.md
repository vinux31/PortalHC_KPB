...
Last activity: 2026-06-18 -- Phase 396 Plan 01 complete (failing InjectExcelHelper unit suite + DTO/VM contracts)
...
**Phase 396 Plan 01 SELESAI (Wave 0 TDD lock, RED)** — 2 commits: `bd3da83c` feat (DTO `InjectExcelUploadResult`+`InjectExcelPreviewRow`+`InjectRequest.EssayTextRequired` default true + VM `Step5Method`) + `cecf9d2f` test RED (`HcPortal.Tests/InjectExcelHelperTests.cs`, 8 fact, pure unit, no DB, no Integration trait). SUMMARY @ `.planning/phases/396-import-excel-retire-bulkbackfill/396-01-SUMMARY.md`. `dotnet build HcPortal.csproj` 0 error; **0 migration**. Suite RED bersih: 108 compile-error SEMUA di file test (CS0103 `InjectExcelHelper` absent + turunan deconstruction), 0 error di luar — RED yang BENAR (tipe hilang, bukan syntax).

**Kontrak Plan 02 yang dikunci:** `InjectExcelHelper.GenerateTemplate(IReadOnlyList<InjectQuestionSpec> questions, IReadOnlyList<(string Nip,string Name)> workers)→XLWorkbook` + `ParseMatrix(Stream, questions, IReadOnlySet<string> allowedNips, IReadOnlyDictionary<string,string> nipToUserId)→(List<InjectWorkerAnswersVM> Workers, List<InjectRowError> Errors, int SkippedBlank)`. Template layout: sheet `"Jawaban"`, kol1=NIP kol2=Nama kol3+=soal `OrderBy(Order).ThenBy(TempId)`, row2+ per-worker; Essay = kolom skor + kolom teks di scoreCol+1 (teks opsional D-05); huruf A=Options[0] (authored, BUKAN OrderBy(TempId)).

**NEXT: `/gsd-execute-phase 396` lanjut Plan 02** — implement `Helpers/InjectExcelHelper.cs` (static EF-free, analog `AssessmentScoreAggregator` + ClosedXML gen/parse `TrainingAdminController:1159/861`) → suite GREEN + restore fast suite (verification §3). Lalu Plan 03 (controller endpoints), Plan 04 (view Step5Method toggle), Plan 05 (retire BulkBackfill + test). 0 migration sepanjang fase. Branch main. ❌ tidak ada edit Dev/Prod.
