...
status: In progress
stopped_at: Completed 387-01-PLAN.md (PXF-06 WR-01/WR-02 + PXF-08 cert-retry + PXF-09 Excel essay cell + PXF-10 monitor broadcast — build 0 error + fast suite 347/347 GREEN, 0 migration; commits 9ccd9a17 / 3b4ce043 / c459c047)
...
Next action: **v31.0 must-fix pra-ujian SELESAI lokal (Phase 385 + 386 keduanya complete + verified).** Sisa handoff (CLAUDE.md Develop Workflow): (1) push 1× bundle 385+386 ke `origin/ITHandoff` (belum dipush oleh developer per instruksi); (2) notify IT — commit hash HEAD + **flag migration = FALSE** (v31.0 = 0 migration baru; carry lama 360/372 tetap pending); (3) IT re-deploy Dev (10.55.3.3) + **UAT browser PXF-01** (gambar sub-path `/KPB-PortalHC`, open blocker — lokal no-repro) sebelum hari-H ~2026-06-17. ❌ JANGAN edit DB/kode Dev/Prod. Pasca-acara: **`/gsd-execute-phase 387`** (Post-Lisensor Polish PXF-06..14, depends 386 kini unblocked) = deploy IT KEDUA.
