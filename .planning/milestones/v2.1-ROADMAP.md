...
Plans:
- [ ] 41-01-PLAN.md — Backend: harden SaveAnswer with ExecuteUpdateAsync atomic upsert; UNIQUE constraint on PackageUserResponse(SessionId, QuestionId); new SaveLegacyAnswer endpoint for legacy exam path
- [ ] 41-02-PLAN.md — Frontend: debounce(300ms) radio handler, save indicator (bottom-right), navigation blocking (5s timeout), 1x retry + toast on failure, "Semua jawaban sudah tersimpan" badge on ExamSummary
