...
- [x] **Phase 41: Auto-Save** — Worker answers are saved to the server automatically on each radio selection and before each page navigation, with atomic upsert preventing data corruption under concurrent requests
- [x] **Phase 42: Session Resume** — A worker who disconnects mid-exam resumes at their last page with pre-populated answers and server-calculated remaining time, not a fresh exam start
...
Plans:
- [x] 41-01-PLAN.md — Backend: harden SaveAnswer with ExecuteUpdateAsync atomic upsert; UNIQUE constraint on PackageUserResponse(SessionId, QuestionId); new SaveLegacyAnswer endpoint for legacy exam path
- [x] 41-02-PLAN.md — Frontend: debounce(300ms) radio handler, save indicator (bottom-right), navigation blocking (5s timeout), 1x retry + toast on failure, "Semua jawaban sudah tersimpan" badge on ExamSummary
...
Plans:
- [ ] 42-01-PLAN.md — DB migration: ElapsedSeconds + LastActivePage on AssessmentSession; SavedQuestionCount on UserPackageAssignment
- [ ] 42-02-PLAN.md — Backend: UpdateSessionProgress POST endpoint; StartExam GET resume detection, stale check, saved answers JSON, remaining time, all ViewBag flags
- [ ] 42-03-PLAN.md — Frontend: Assessment.cshtml Resume button (btn-warning); StartExam.cshtml resume modal, timer init, answer pre-population, periodic save (30s + nav), expired exam path
- [ ] 42-04-PLAN.md — Human verification: end-to-end resume flow (button, modal, page, timer, answers, progress save)
...
|-------------|-------|--------|
| SAVE-01 | Phase 41 | Complete |
| SAVE-02 | Phase 41 | Complete |
| SAVE-03 | Phase 41 | Complete |
