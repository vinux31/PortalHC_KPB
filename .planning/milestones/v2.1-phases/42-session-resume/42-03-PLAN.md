...
must_haves:
  truths:
    ...
    - "If pre-populate loads no answers AND IS_RESUME is true: toast 'Gagal memuat jawaban sebelumnya. Lanjutkan dari soal no. X.' is shown"
  artifacts:
...
**CHANGE 5 — Add answer pre-population function with failure toast (insert before the existing `// -------- Radio change listener` comment, around line 351):**

```javascript
// -------- Answer pre-population (for resume) --------
function prePopulateAnswers() {
    if (!IS_RESUME) return;   // Only relevant for resume sessions

    const hasAnswers = SAVED_ANSWERS && Object.keys(SAVED_ANSWERS).length > 0;

    if (!hasAnswers) {
        // Locked decision: show toast warning when pre-populate has no data on resume
        // "Gagal memuat jawaban sebelumnya. Lanjutkan dari soal no. X."
        // X is 1-based: RESUME_PAGE is 0-indexed, so display as (RESUME_PAGE + 1)
        showResumeFailureToast('Gagal memuat jawaban sebelumnya. Lanjutkan dari soal no. ' + (RESUME_PAGE + 1) + '.');
        return;
    }
...
// Show a non-blocking toast warning for pre-populate failure
// Uses inline Bootstrap toast (no external library needed — Bootstrap 5 already loaded)
function showResumeFailureToast(message) {
    ...
    if (!toastContainer) {
        ...
        toastContainer.style.cssText = 'position:fixed;bottom:1.5rem;end:1rem;z-index:1100;';
        // Use Bootstrap's position utility classes instead of inline style for RTL safety
        toastContainer.className = 'position-fixed bottom-0 end-0 p-3';
        toastContainer.style.zIndex = '1100';
        document.body.appendChild(toastContainer);
    }
    const toastId = 'resumeFailureToast';
    const toastHtml = '<div id="' + toastId + '" class="toast align-items-center text-bg-warning border-0" role="alert" aria-live="assertive" aria-atomic="true" data-bs-autohide="true" data-bs-delay="6000">' +
        '<div class="d-flex">' +
        '<div class="toast-body fw-semibold"><i class="bi bi-exclamation-triangle-fill me-2"></i>' + message + '</div>' +
...
**Key behavior:**
- `prePopulateAnswers()` returns immediately if `IS_RESUME` is false (fresh start — no toast, no work needed).
- If IS_RESUME is true but SAVED_ANSWERS is empty or has zero entries: shows the failure toast per the locked user decision, then returns. The exam still opens on the saved page (the init block handles navigation separately).
- If IS_RESUME is true and SAVED_ANSWERS has data: pre-checks all radios, fills hidden inputs, updates answeredQuestions Set, calls updateAnsweredCount().
- Toast uses Bootstrap 5's `bootstrap.Toast` (already loaded in the layout). Auto-hides after 6 seconds. Uses `text-bg-warning` for visual consistency with the warning theme.
- Toast message: "Gagal memuat jawaban sebelumnya. Lanjutkan dari soal no. X." where X = RESUME_PAGE + 1 (1-based page number per user decision).

---

**CHANGE 6 — Add UpdateSessionProgress save function (insert after prePopulateAnswers/showResumeFailureToast, before radio listener):**
...
// Also shows failure toast if IS_RESUME is true but no answers were loaded
prePopulateAnswers();
...
**Behavioral notes:**
- `prePopulateAnswers()` runs first (before any modal), ensuring `answeredQuestions` Set is filled and count badge is accurate immediately. If IS_RESUME is true but SAVED_ANSWERS is empty, the failure toast is shown here.
...
2. Read Views/CMP/StartExam.cshtml and confirm:
   ...
   - `prePopulateAnswers()` function exists with IS_RESUME guard at top
   - `prePopulateAnswers()` calls `showResumeFailureToast(...)` when `!hasAnswers` and IS_RESUME is true
   - `showResumeFailureToast()` function exists, creates Bootstrap Toast with `text-bg-warning`
   - Toast message contains "Gagal memuat jawaban sebelumnya. Lanjutkan dari soal no."
  ...
  <done>Assessment.cshtml shows Resume button for InProgress sessions. StartExam.cshtml: resume modal with single "Lanjutkan" button, timer starts at REMAINING_SECONDS_FROM_DB, prePopulateAnswers fills all radio buttons and answered count before render (or shows failure toast "Gagal memuat jawaban sebelumnya. Lanjutkan dari soal no. X." when IS_RESUME is true but SAVED_ANSWERS is empty), UpdateSessionProgress called on nav + every 30s, expired path auto-submits with modal. Build: 0 errors.</done>
</task>
...
- Grep Views/CMP/StartExam.cshtml for "showResumeFailureToast" — finds function definition + call inside prePopulateAnswers when !hasAnswers
- Grep Views/CMP/StartExam.cshtml for "Gagal memuat jawaban" — finds the exact toast message text
...
<success_criteria>
Complete session resume frontend implemented. Workers see Resume button in yellow on assignment cards. Resume modal appears before exam loads. Timer counts from remaining time. All previous answers pre-checked. If pre-populate has no data and IS_RESUME is true, toast "Gagal memuat jawaban sebelumnya. Lanjutkan dari soal no. X." appears (per locked user decision). Progress saved every 30s + on navigation. Expired exam auto-submits. No deferred ideas implemented (no mobile-specific UI, no auto-recovery for stale questions).
</success_criteria>
