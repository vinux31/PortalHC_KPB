...
- **D-05:** Guard helper (A-M11): jalan HANYA kalau `Category=="Assessment Proton"` && `IsPassed` && `ProtonTrackId.HasValue`. Jangan kepicu di exam non-Proton/Pre-Test.
- **D-05a (REVISED via research+DB 2026-06-10):** Spec §4.9 klaim "Proton=Standard-only" TIDAK di-enforce kode — `GradeAndCompleteAsync` early-return di cabang `hasEssay` (`GradingService.cs:189-227`) tanpa capai hook penanda; IsPassed Essay final di-set di `FinalizeEssayGrading` (`AssessmentAdminController.cs:3566`) yang tak punya hook. DB lokal `HcPortalDB_Dev` verifikasi: 3 sesi Proton (1 Tahun1 + 2 Tahun3) SEMUA `HasManualGrading=0` (0 essay) → aturan bisnis terpenuhi di data sekarang. **Keputusan user: Opsi A — tambah defensive hook di `FinalizeEssayGrading`** (Proton + IsPassed=true → `EnsureAsync(Origin="Exam")`). Hook praktis idle karena Essay tak ada di Proton, tapi nutup celah bug-diam Dev/Prod/masa-depan. Scope 358 = hook di GradeAndCompleteAsync (pilihan-ganda) + RegradeAfterEditAsync (flip) + FinalizeEssayGrading (defensive).
