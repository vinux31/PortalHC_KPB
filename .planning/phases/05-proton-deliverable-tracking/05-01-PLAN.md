...
1. **ProtonMainViewModel** — For ProtonMain page (PROTN-01):
   ...
   - `List<ProtonDeliverableProgress> ActiveProgresses = new()` — active progress records for each coachee (Status == "Active"), used to render "Lihat Deliverable" link buttons per coachee row
...
2. **ProtonPlanViewModel** — For PlanIdp Coachee view (PROTN-02):
   ...
   - `ProtonDeliverableProgress? ActiveProgress` — the coachee's current active deliverable progress record (Status == "Active"), used to render a navigation button to the Deliverable page
...
Use `namespace HcPortal.Models;` (file-scoped). Follow existing property conventions (PascalCase, nullable reference types with `?`).
  ...
  <verify>
`dotnet build -c Release` succeeds with 0 errors. Grep confirms all 5 entity class names exist in ProtonModels.cs. Grep confirms all 3 ViewModel class names exist in ProtonViewModels.cs. Grep confirms `ActiveProgress` property exists in ProtonPlanViewModel. Grep confirms `ActiveProgresses` property exists in ProtonMainViewModel.
  </verify>
  <done>All 5 Proton entity classes and 3 ViewModels exist with correct properties, navigation properties, and defaults. ProtonPlanViewModel includes ActiveProgress property. ProtonMainViewModel includes ActiveProgresses list.</done>
</task>
...
6. ProtonPlanViewModel contains ActiveProgress property
7. ProtonMainViewModel contains ActiveProgresses list property
</verification>
...
- ProtonPlanViewModel.ActiveProgress and ProtonMainViewModel.ActiveProgresses properties exist for navigation support
</success_criteria>
