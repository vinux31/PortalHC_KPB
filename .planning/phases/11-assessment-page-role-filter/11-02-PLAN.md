...
**Step A — Keep the existing header toggle unchanged (lines 12-53):**

Do NOT modify the header section. The existing Personal View / Manage View toggle buttons (lines 36-52) remain exactly as they are. HC/Admin users who enter with `view=manage` see the "Personal View" button to switch to their own assignments; workers or HC/Admin in personal mode see the "Manage View" button to switch back. This preserves backward compatibility and the `view="manage"` entry point.
...
Copy the existing card rendering loop from Assessment.cshtml lines 131-317 into the `#management-tab-pane` div. This is the `@foreach (var item in Model)` block that renders the full card grid with category badges, title, meta info, manage-info, and action buttons. Make these specific changes when copying:

1. Change the iteration from `@foreach (var item in Model)` to `@foreach (var item in managementData)` — this uses the ViewBag.ManagementData list cast in Step B instead of the page Model.

2. Remove the `@if (viewMode == "manage")` / `else` branching inside the card body (original lines 223-313). Since this is the Management tab, it is ALWAYS the manage view. Keep ONLY the manage-view branch content (the `manage-info` div with assigned-to/token/schedule at lines 226-245, and the management action buttons at lines 248-268). Remove the personal-view `else` branch (lines 271-312) entirely — workers never see this tab.

3. Keep everything else identical: the outer `<div class="row g-4">`, the `<div class="col-12 col-md-6 col-lg-4 assessment-card" data-status="...">` wrapper, the card header with category badge (lines 158-161), the title (lines 163-166), the assessment-meta section with date/duration/status/token-required (lines 168-221).

The result inside `#management-tab-pane` should be:
...
<div class="row g-4">
    ...
    else
    {
        @foreach (var item in managementData)
        {
            ...
            <div class="col-12 col-md-6 col-lg-4 assessment-card" data-status="@item.Status.ToLower()">
                    ...
                    <div class="card-body d-flex flex-column p-4">
                        ...
                        <div class="assessment-meta mb-3">
                            ...
                            <div class="meta-item">
                                ...
                                <span>
                                    @{
                                        var hours = item.DurationMinutes / 60;
                                        var minutes = item.DurationMinutes % 60;
                        ...
                        <!-- Manage Info (always shown — this IS the management tab) -->
                        <div class="manage-info mb-3">
                            <div class="d-flex align-items-center text-muted small mb-2">
                                <i class="bi bi-person me-2"></i>
                                <span><strong>Assigned to:</strong> @(item.User?.FullName ?? "Unknown")</span>
                            </div>
                            @if (item.IsTokenRequired && !string.IsNullOrEmpty(item.AccessToken))
                            {
                                <div class="d-flex align-items-center text-muted small mb-2">
                                    <i class="bi bi-key me-2"></i>
                                    <span><strong>Token:</strong> <code id="token-@item.Id" class="text-primary">@item.AccessToken</code></span>
                                    <button class="btn btn-sm btn-link p-0 ms-2" onclick="copyToken('@item.AccessToken', @item.Id, event)" title="Copy Token">
                            ...
                            <div class="d-flex align-items-center text-muted small">
                                ...
                                <span><strong>Schedule:</strong> @item.Schedule.ToString("dd MMM yyyy")</span>
                            </div>
                        </div>
                        <!-- Management Actions -->
                        <div class="mt-auto">
                            <div class="d-flex gap-2 flex-wrap">
                                <a asp-action="EditAssessment" asp-route-id="@item.Id" class="btn btn-sm btn-outline-primary flex-fill">
                                    <i class="bi bi-pencil"></i> Edit
                                </a>
                                <a asp-action="ManageQuestions" asp-route-id="@item.Id" class="btn btn-sm btn-outline-info flex-fill">
                            ...
                            <div class="d-flex gap-2 flex-wrap mt-2">
                                @if (item.IsTokenRequired)
                                {
                                    <button class="btn btn-sm btn-outline-warning flex-fill" onclick="regenerateToken(@item.Id)">
                                        <i class="bi bi-arrow-clockwise"></i> Regen Token
                                    </button>
                                }
                                <button class="btn btn-sm btn-outline-danger flex-fill" onclick="confirmDelete(@item.Id, '@item.Title')">
