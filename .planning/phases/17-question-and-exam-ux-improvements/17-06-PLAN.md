...
public async Task<IActionResult> ExamSummary(int id)
{
    ...
    // Correct unboxing: TempData stores int as boxed object; 'as int?' cannot unbox, use pattern matching
    int? assignmentId = TempData["PendingAssignmentId"] is int val ? val : (int?)null;
...
if (packageAssignment != null)
{
    ...
    foreach (var q in packageQuestions)
    {
        ...
        // Save UserResponse (reuse existing UserResponse entity; AssessmentQuestionId stores PackageQuestion.Id pragmatically)
...
3. Results page shows correct score and pass/fail
  ...
  <done>
ExamSummary POST + GET actions added. SubmitExam POST wraps legacy grading in `else` branch — package path and legacy path are mutually exclusive. TempData unboxing uses pattern matching (`is int val`). ExamSummaryItem class added to PackageExamViewModel.cs. Project builds.
  </done>
</task>
