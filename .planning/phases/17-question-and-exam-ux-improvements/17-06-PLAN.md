...
public async Task<IActionResult> ExamSummary(int id)
{
    ...
    int? assignmentId = TempData["PendingAssignmentId"] as int?;
...
The existing SubmitExam POST grades using `AssessmentQuestion` + `AssessmentOption`. Add a new branch before the existing logic to handle the package path:

At the top of the existing `SubmitExam` POST, after the authorization/completion checks, add:
...
if (packageAssignment != null)
{
    ...
    foreach (var q in packageQuestions)
    {
        ...
        maxScore += 0; // already counted above
        ...
        // Save UserResponse (reuse existing UserResponse entity, SelectedOptionId stores PackageOption.Id)
        // Note: AssessmentQuestionId stores the PackageQuestion.Id here — this is a pragmatic reuse.
        // Future: may want a separate PackageUserResponse table.
        _context.UserResponses.Add(new UserResponse
        {
            AssessmentSessionId = id,
            AssessmentQuestionId = q.Id,       // stores PackageQuestion.Id
            SelectedOptionId = selectedOptId   // stores PackageOption.Id
        });
    }
...
// else: fall through to existing legacy grading logic below
```

**Note on UserResponse reuse:** Using existing `UserResponse` entity to store package responses is pragmatic — the FK on `AssessmentQuestionId` is named poorly but stores `PackageQuestion.Id` values in the package path. The Results view already conditionally shows answer review, and in the package path it will work as long as `AllowAnswerReview` is false OR the Results view is updated. For this phase, the Results page will show scores/pass-fail correctly; full answer review for package path is deferred.
...
3. Results page shows correct score and pass/fail
  ...
  <done>
ExamSummary POST + GET actions added. SubmitExam POST handles package path with ID-based grading. ExamSummaryItem class added to PackageExamViewModel.cs. Project builds.
  </done>
</task>
...
- SubmitExam grades by PackageOption.IsCorrect (not letter-based), stores score correctly
