...
The ExamSummary GET action loads the assessment at line 2101 with `FindAsync(id)`. The `Schedule` property is a plain `DateTime` column on `AssessmentSession`, so it is already available on the loaded entity -- no `.Include()` is needed.

After the Forbid check (~line 2105) and before the legacy else block (~line 2161), add the sibling session lookup. Place it just before the `if (assignment != null)` check at line 2131 so it is in scope for both branches, OR place it inside the else block just before the legacyQuestions query. Either location is fine as long as `siblingSessionIds` is in scope when the legacy query executes:
...
Do NOT change the `FindAsync(id)` call at line 2101 -- `assessment.Schedule.Date` is already accessible on the loaded entity without any Include.

**FIX 2 -- SubmitExam POST (~line 2272):**

The SubmitExam action loads assessment at line 2274 with `.Include(a => a.Questions).ThenInclude(q => q.Options).FirstOrDefaultAsync(a => a.Id == id)`. For the worker's own session, `assessment.Questions` is empty. The `Schedule` property is a plain `DateTime` column, so it is already available on the loaded entity -- do NOT add `.Include(a => a.Schedule)`.

Inside the else block at line 2406, before the grading loop, add the sibling session lookup and load questions from the sibling:
...
Do NOT change the existing assessment loading query at line 2274 -- `assessment.Schedule.Date` is already accessible without any Include.
...
6. No .Include(a => a.Schedule) anywhere -- Schedule is a DateTime column, not a navigation property
</verification>
