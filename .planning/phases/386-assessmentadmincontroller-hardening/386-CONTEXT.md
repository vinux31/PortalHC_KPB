...
- **⚠️ MA mislabel surface KEDUA (BARU, verifier PXF05-F1) — `Helpers/ExcelExportHelper.cs:83` `AddDetailPerSoalSheet`** (dipanggil controller `~4690`, sheet "Detail Per Soal" pada Summary-Excel) pakai `FirstOrDefault` 1-row → MA salah-label + jawaban tak lengkap, **bug-class SAMA dgn PXF-05**. Beda file (Helpers, bukan controller) → **OUT single-file phase ini**, TAPI ekspor resmi lisensor juga. **Keputusan owner:** fix bareng (perluas scope ke ExcelExportHelper) atau Future. **Sudah dicatat ke readiness register** (lihat note). Beda dari Excel per-session `~4863` yang sudah benar.
...
- **`ImportPackageQuestions` (`~6162-6230`)** wajibkan ke-4 opsi terisi → **tak bisa hasilkan 0-opsi** (aman dari F-DEV-01). TAPI aturan MA-benar import (`~6179` `≥1` benar) BEDA dari form (`≥2`). Pasca D-01: deklarasikan importer OUT eksplisit ATAU samakan rule MA import → `≥2` (hindari split-brain).
