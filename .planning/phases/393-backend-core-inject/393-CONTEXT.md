...
- **D-12:** Nomor cert **auto-generate** memakai **tanggal ujian (backdate `CompletedAt`)** untuk basis ROMAN-bulan + tahun + sekuens — bukan `DateTime.Now`. Mis. ujian Juni 2026 → `KPB/xxx/VI/2026`. **Deviasi sadar** dari jalur online (yang pakai `DateTime.Now` karena di online "now" = waktu ujian): inject WAJIB melewatkan `session.CompletedAt` (backdate) ke `CertNumberHelper.Build` + `GetNextSeqAsync(year)` pakai tahun backdate. Setia "seakan online" historis. (Sumber: RESEARCH.md assumption A1 — dikonfirmasi user 2026-06-17.) Escape: mode cert manual (D-09) untuk nomor luring custom.
