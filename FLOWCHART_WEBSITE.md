...
flowchart TB
    ...
    subgraph CMP["📋 Competency Management Platform"]
    ...
    subgraph CDP["📈 Competency Development Platform"]
...
## 📋 Modul 1: CMP (Competency Management Platform)
...
## 📈 Modul 2: CDP (Competency Development Platform)
...
flowchart TD
    ...
    CHECK_ROLE -->|Section Head/Sr Supervisor| FILTER_FLOW["Cascading Filter:<br/>Bagian → Unit → Level"]
...
## 👤 Modul 3: BP (Business Partner Platform)
...
flowchart TD
    ...
    CHECK -->|Ya| PASS["✅ Eligible"]
    CHECK -->|Tidak| FAIL["❌ Not Eligible"]
...
## 🔒 Role-Based Access Control (Detail)
...
PortalHC_KPB/
├── Controllers/
│   ├── AccountController.cs    → Login, Logout, Profile, Settings
│   ├── BPController.cs         → TalentProfile, PointSystem, EligibilityValidator
│   ├── CDPController.cs        → IDP, Coaching, Progress, Dashboard
│   ├── CMPController.cs        → KKJ, Mapping, Assessment, Records
│   └── HomeController.cs       → Home Page
├── Views/
│   ├── Account/                → Login.cshtml, Profile.cshtml, Settings.cshtml
│   ├── BP/                     → TalentProfile.cshtml, PointSystem.cshtml, etc.
│   ├── CDP/                    → Index.cshtml, Coaching.cshtml, Progress.cshtml
│   ├── CMP/                    → Kkj.cshtml, Assessment.cshtml, Records.cshtml
│   ├── Home/                   → Index.cshtml
│   └── Shared/                 → _Layout.cshtml (Navigation)
├── Models/
│   ├── ApplicationUser.cs      → Extended Identity User
│   ├── UserRoles.cs            → Role Constants & Helpers
│   ├── TrainingRecord.cs       → Training Data Model
│   ├── CoachingLog.cs          → Coaching Data Model
│   └── ...
├── Data/
│   └── ApplicationDbContext.cs → EF Core DbContext
└── HcPortal.db                 → SQLite Database
```
